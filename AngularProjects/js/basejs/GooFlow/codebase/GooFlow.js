...
GooFlow.prototype={
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item .ico","dblclick",{inthis:this},function(e){
			var This=e.data.inthis;
			var id = $(this).parent("tr").parent("tbody").parent("table").parent(".GooFlow_item").attr("id")
			if(!$('.GooFlow_work_inner').children("div:last").hasClass("attr")){
				$('.GooFlow_work_inner').append('<div class="attr" id="attr'+id+'">'+
				'<textarea class="attrText"></textarea>'+
				...
				$("#attr"+id).children("textarea").val(JSON.stringify(This.$nodeData[id].attr))
				$("#attr"+id).on("click",".confirm",function(){
					var val = $("#attr"+id).children("textarea").val()
					try{
						var jsonobj = JSON.parse(val);
						if(jsonobj)
						{
							This.setAttr(id,jsonobj)
							$("#attr"+id).remove()
						} 
					}
					catch(err) {
				...
				$("#attr"+id).on("click",".cancel",function(){
					$("#attr"+id).remove()
				})				
			}
		});
