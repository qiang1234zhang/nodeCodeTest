...
import {getImgPath} from 'src/components/common/mixin'//使用在mixin创建的混入
...
export default {
    ...
    mixins: [getImgPath],//使用在mixin创建的混入
    components:{
        headTop,
        footGuide,
    },
    ...
    computed:{
        ...mapState([
            'userInfo',//映射this.userInfo()为this.$store.commit('userInfo')
        ]),
        //后台会返回两种头像地址格式，分别处理
        imgpath:function () {
            let path;
            if(this.avatar.indexOf('/') !==-1){
                ...
                path = this.getImgPath(this.avatar)//使用在mixin创建的混入
            }
    ...
    methods:{
        ...mapMutations([
            'SAVE_AVANDER',//映射this.SAVE.AVANDER()为this.$store.commit('SAVE_AVANDER')
        ]),
        initData(){
    ...
    watch: {//监听data中的userInfo的发生变化时，执行initData函数
